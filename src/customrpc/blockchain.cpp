#include <customrpc/blockchain.h>
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (GetUTXOStats(coins_view, *blockman, stats, node.customrpc_interruption_point, pindex)) {
        ...
        if (!stats.index_used) {
            ...
            if (pindex->nHeight > 0) {
                GetUTXOStats(coins_view, *blockman, prev_stats, node.customrpc_interruption_point, pindex->pprev);
            }
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (request.params[0].get_str() == "status") {
        ...
        bool res = FindScriptPubKey(g_scan_progress, g_should_abort_scan, count, pcursor.get(), needles, coins, node.customrpc_interruption_point);
...
UniValue CreateUTXOSnapshot(NodeContext& node, CChainState& chainstate, CAutoFile& afile)
{
...

    {
        ...
        if (!GetUTXOStats(&chainstate.CoinsDB(), chainstate.m_blockman, stats, node.customrpc_interruption_point)) {
            throw JSONRPCError(RPC_INTERNAL_ERROR, "Unable to read UTXO set");
        }
    ...
    while (pcursor->Valid()) {
        if (iter % 5000 == 0) node.customrpc_interruption_point();
        ++iter;
